...
def run_exp(train_loader, val_loader, test_loader, num_features, runs, n_layers, early_stop_flag, dropout, model_name, num_epochs, wd, hidden_channels, lr, bias, patience, loss_thresh, data_name, unique_run_id, one_m, normalize_m, num_classes, out_dim, weight, aggregation, tuning):
    np.random.seed(0)
    ...
    with open(log_file, 'w') as log:
        for i, seed in enumerate(seeds):
            ...
            loss = loss_type()
            early_stop = EarlyStopping(patience=patience, min_is_better=True)  # 假
            ...
            for epoch in range(num_epochs):
                ...
                train_loss, train_acc, train_auroc, train_auprc, train_recall, train_precision, train_f1, train_time = \
                    trainer.train_epoch(model, dloader=train_loader, optimizer=optimizer, device=device, loss_fn=loss)
                ...
                if model_name == "HGNAM":
                    val_loss, val_acc, val_auroc, val_auprc, val_rec, val_prec, val_f1, val_time = \
                        trainer.test_epoch(model, dloader=val_loader, device=device, val_mask=True, loss_fn=loss)
                else:
                    val_loss, val_acc, val_auroc, val_auprc, val_rec, val_prec, val_f1, val_time = \
                        trainer.test_epoch(model, dloader=val_loader, device=device, val_mask=False, loss_fn=loss)
...
if __name__ == '__main__':
    ...
    run_exp(train_loader=train_loader, val_loader=val_loader, test_loader=test_loader, num_features=num_features,
            ...
            loss_thresh=loss_thresh, runs=args.runs, data_name=args.data_name,
            unique_run_id=unique_run_id, one_m=args.one_m, normalize_m=args.normalize_m,
            num_classes=num_classes,
            out_dim=out_dim,
            weight=args.weight,aggregation=args.aggregation, tuning=args.tuning)